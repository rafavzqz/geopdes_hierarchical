...
[hmsh, hspace, u_Massimo] = adaptivity_coarsen (hmsh, hspace, marked, adaptivity_data);
hspace.dofs = u_Massimo;

npts = [101 101];
[eu, F] = sp_eval (u_Massimo, hspace, geometry, npts);
figure(1); surf (squeeze(F(1,:,:)), squeeze(F(2,:,:)), eu);
figure(2);
fig_mesh = hmsh_plot_cells (hmsh, 20, 2 );
