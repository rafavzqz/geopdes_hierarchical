...
method_data.nsub_coarse = [2^2 2^2 2^2];               % Number of subdivisions of the coarsest mesh, with respect to the mesh in geometry
...
adaptivity_data.crp = 2;                            %balancing parameter
adaptivity_data.radius = [0.01, 0.01, 0.0025]*1.0;
...
plot_data.time_steps_to_post_process = [1,5,10,15,20,25,30,35,40,45,50]; %[1,25,50,75,100,101,150,200,250,300,350,400]; % [101,200];%linspace(1,n_time_steps,n_time_steps); %200; %  
plot_data.file_name = strcat(problem_output.folder, '/Fachinotti_50_Depth5_C1_P2_3D_Neumann_balanced%d.vts');
plot_data.file_name_mesh = strcat(problem_output.folder, '/Fachinotti_50_Depth5_C1_P2_3D_Neumann_mesh_balanced%d');
plot_data.file_name_dofs = strcat(problem_output.folder, '/Fachinotti_50_Depth5_C1_P2_3D_Neumann_dofs_balanced');
% plot_data.file_name_temp_plot = strcat(problem_output.folder, '/Fachinotti_20_maxDepth6_P2_3D_lumped_temperature_%d');
...
fid = fopen (strcat(problem_output.folder, '/Fachinotti_50_Depth5_C1_P2_3D_Neumann_time_balanced'), 'w');
tic
