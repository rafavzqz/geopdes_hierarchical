...
problem_data.geo_name = gsTHBSpline(filename);
...
problem_data.grad_c_diff = @(x,y) reshape (zeros(size(x)), [1, size(x)]);
...
problem_data.graduex = @(x,y) [2*pi*cos(2*pi*x).*sin(2*pi*y); 2*pi*sin(2*pi*x).*cos(2*pi*y)];
...
% EXPORT VTK FILE
npts = [51 51];
output_file = 'laplace_adaptivity_square_gismo.vts';
