% OP_F_V_HIER_VECTOR: assemble the right-hand side vector r = [r(i)], with  r(i) = (f, v_i) for hierarchical splines, 
%  exploiting the multilevel structure.
%
%   rhs = op_f_v_hier_vector (hspace, hmsh, coeff, [patches]);
%
% INPUT:
%     
...
function rhs = op_f_v_hier_vector (hspace, hmsh, f, patch_list)
  ...
  for ilev = 1:hmsh.nlevels
    ...
    if (hmsh.nel_per_level(ilev) > 0)
      ...
      if (msh_lev.nel > 0)
        ...
        sp_lev = sp_scalar2vector (sp_lev, msh_lev, 'value', true);

        b_lev = op_f_v (sp_lev, msh_lev, f(x{:}));

        dofs = [];
        for icomp = 1:rdim
          dofs = union (dofs, (icomp-1)*hspace.ndof + (1:ndofs));
        end
        
        Csub = repmat (hspace.Csub(ilev), 1, rdim);
        Csub = blkdiag (Csub{:});
        rhs(dofs) = rhs(dofs) + Csub.' * b_lev;
      end
    end
  end
