...
% according to patch_reorientation, without using three nested if-else.
  ...
  for iv = 1:nvert
    ...
    % To deal with the orientation we have to choose the correct indices on Lambda
    for ip = 1:numel(patches)
      ...
      if (sp_coarse.vertices(iv).edge_orientation(ip) == 1)
        Lambda = [Proj0_patch(4:dim_sp0_ref-3,[1 2 3]), zeros(dim_sp0_ref-6,2);...
      ...
      if (nargin == 5)
        C(ind_edge_ref,indices_v_coarse) = Lambda*K_prev;
      elseif (nargin == 6)
        [ind_coarse_on_vertex,local_indices,~] = intersect (indices_v_coarse, ind_coarse);
        C(ind_edge_ref,ind_coarse_on_vertex) = Lambda*K_prev(:,local_indices);
      elseif (nargin == 7)
        [~,local_indices_c,ind_c] = intersect (indices_v_coarse, ind_coarse);
        [~,local_indices_f,ind_f] = intersect (ind_edge_ref, ind_fine);
        C(ind_f, ind_c) = Lambda(local_indices_f,:)*K_prev(:,local_indices_c);
      end
   ...
   %the dependence on the corresponding edge functions
    if (numel(edges) > numel(patches))
      ...
      elseif (nargin == 6)
        [ind_coarse_on_vertex,local_indices,~] = intersect (indices_v_coarse, ind_coarse);
        C(ind_edge_ref,ind_coarse_on_vertex) = Lambda*K_next(:,local_indices);
      elseif (nargin == 7)
        [~,local_indices_c,ind_c] = intersect (indices_v_coarse, ind_coarse);
        [~,local_indices_f,ind_f] = intersect (ind_edge_ref, ind_fine);
        C(ind_f, ind_c) = Lambda(local_indices_f,:)*K_next(:,local_indices_c);
      end
    end 
  end
