...
if (numel(hspace.space_of_level) == hmsh.nlevels-1)
  ...
  [new_space, Proj, Proj0, Proj1] = sp_refine (hspace.space_of_level(hmsh.nlevels-1), msh_level, hmsh.nsub, degree, regularity);
  hspace.space_of_level(hmsh.nlevels) = new_space; clear new_space
  hspace.Proj(hmsh.nlevels-1,:) = Proj(:);
  hspace.Proj0(hmsh.nlevels-1,:) = Proj0(:);
  hspace.Proj1(hmsh.nlevels-1,:) = Proj1(:);
