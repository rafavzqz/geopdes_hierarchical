...
for ilev = hspace.nlevels+1:hmsh_fine.nlevels
  ...
  [new_space, Proj, Proj0, Proj1] = sp_refine (hspace.space_of_level(ilev-1), msh_level, hmsh_fine.nsub); %, degree, degree-1);
  hspace.space_of_level(ilev) = new_space; clear new_space
  hspace.Proj(ilev-1,:) = Proj(:);
  hspace.Proj0(ilev-1,:) = Proj0(:);
  hspace.Proj1(ilev-1,:) = Proj1(:);
