...
%     option:    accepted options are 'value' (default), 'gradient', 'laplacian', 'bilaplacian'
%                'hessian', 'curl', 'divergence', 'third_derivative', 'fourth_derivative'
%
% OUTPUT:
%
...
% For vector-valued spaces, the value of catdir is then corrected by adding one
  ...
  third_derivative = false; fourth_derivative = false;
  for iopt = 1:nopts
    switch (lower (options{iopt}))
      ...
      case 'laplacian' % Only for scalars
        laplacian = true;
        catdir(iopt) = 2;
      case 'bilaplacian' % Only for scalars
        bilaplacian = true;
        catdir(iopt) = 2;
      case 'hessian'
        hessian = true;
      ...
      case 'divergence' % Only for vectors
        divergence = true;
        catdir(iopt) = 1;
      case 'third_derivative' % Only for vectors
        third_derivative = true;
        catdir(iopt) = 5;
      case 'fourth_derivative' % Only for vectors
        third_derivative = true;
        catdir(iopt) = 6;
      otherwise
  ...
  if (hspace.ncomp ~= 1)
    ...
    eval_element_list = @(SP, MSH) sp_evaluate_element_list (SP, MSH, ...
        'value', value, 'gradient', gradient, 'hessian', hessian, 'curl', curl, ...
        'divergence', divergence, 'third_derivative', third_derivative, ...
        'fourth_derivative', fourth_derivative);
  else
    eval_element_list = @(SP, MSH) sp_evaluate_element_list (SP, MSH, ...
        'value', value, 'gradient', gradient, 'laplacian', laplacian, ...
        'hessian', hessian, 'bilaplacian', bilaplacian, 'third_derivative', third_derivative, ...,
        'fourth_derivative', fourth_derivative);
  end
