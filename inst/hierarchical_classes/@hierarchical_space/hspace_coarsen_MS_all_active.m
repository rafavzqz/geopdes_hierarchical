...
% HERE I IMPLEMENTED THE Speleers-Manni QI
% I use a local L2 projection
% The interval for each function is chosen as all the active elements of
%  its level within the support

function [hspace, Ccoarse] = hspace_coarsen_MS_all_active (hspace, hmsh, FtR, removed_cells)
...
[~, inew, iold] = intersect (active{hspace.nlevels}, hspace.active{hspace.nlevels});
ndof_until_lev_old = sum (hspace.ndof_per_level(1:hspace.nlevels-1));
ndof_until_lev = sum (cellfun (@numel, active(1:hspace.nlevels-1)));
...
for lev = hspace.nlevels-1:-1:1
  ...
  ndof_until_lev_old = sum (hspace.ndof_per_level(1:lev));
  [all_cells, cells_per_function] = ...
      sp_get_cells (hspace.space_of_level(lev), hmsh.mesh_of_level(lev), active{lev});
  ...
  msh_next_level = msh_evaluate_element_list (hmsh.mesh_of_level(lev+1), removed_cells{lev+1});

  for ifun = 1:numel(active{lev})
...
% Computation in cells that were previously active
    previous_cells = intersect (old_active_cells, cells);
    msh_previous = msh_restrict_to_cells (hmsh.msh_lev{lev}, previous_cells);
    sp_previous = sp_evaluate_element_list (hspace.space_of_level(lev), msh_previous);

    sp_previous_loc = sp_previous;
    [~,position] = ismember (sp_previous_loc.connectivity, indices);
    sp_previous_loc.ndof = numel (indices);
...
%  children, as they involve functions of level lev+1
    ...
    msh_children = msh_restrict_to_cells (msh_next_level, children);
    sp_children = sp_evaluate_element_list (hspace.space_of_level(lev+1), msh_children);
...
%  otherwise some coefficients are set to zero
    ...
    Gprev2 = op_u_v (sp_children, sp_children, msh_children, ones (msh_children.nqn, msh_children.nel));
    Gprev2 = Cproj.' * Gprev2 * hspace.Csub{lev+1};

% %%%% Alternative for Gprev2. May be clearer, but it is time consuming
%     new_cells = setdiff (cells, old_active_cells);
%     children = hmsh_get_children (hmsh, lev, new_cells);
%     msh_children = msh_restrict_to_cells (msh_lev1, children);
%     space_aux = hspace.space_of_level(lev).constructor(hmsh.mesh_of_level(lev+1));
%     space_aux = sp_evaluate_element_list (space_aux, msh_children);
%     Gprev3 = op_u_v (sp_lev1, space_aux, msh_children, ones (msh_children.nqn, msh_children.nel));
%     Gprev3 = Gprev3(indices,:) * hspace.Csub{lev+1};
    ...
    aux (abs(aux) < 1e-12) = 0;
    index = find (indices == active{lev}(ifun));
    Ccoarse(ndof_until_lev+ifun,unique(jj)) = aux(index,:);
  end
end
