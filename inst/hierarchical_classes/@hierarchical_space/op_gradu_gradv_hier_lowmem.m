...
function varargout = op_gradu_gradv_hier_lowmem (hspu, hspv, hmsh, coeff)
  ...
  for ilev = 1:hmsh.nlevels
    ...
    if (hmsh.nel_per_level(ilev) > 0)
      active_elems = hmsh.active{ilev};
      for iel = 1:numel(active_elems)
        msh_aux = msh_restrict_to_cells (hmsh.msh_lev{ilev}, active_elems(iel));
      
        x = cell (hmsh.rdim,1);
        for idim = 1:hmsh.rdim
          x{idim} = reshape (msh_aux.geo_map(idim,:,:), msh_aux.nqn, msh_aux.nel);
        end
      
        spu_lev = sp_evaluate_element_list (hspu.space_of_level(ilev), msh_aux, 'value', false, 'gradient', true);
        spv_lev = sp_evaluate_element_list (hspv.space_of_level(ilev), msh_aux, 'value', false, 'gradient', true);
        K_lev = op_gradu_gradv (spu_lev, spv_lev, msh_aux, coeff (x{:}));
