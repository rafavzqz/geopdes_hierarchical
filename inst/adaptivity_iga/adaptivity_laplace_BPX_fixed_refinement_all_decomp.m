...
function  [geometry, hmsh, hspace, u, solution_data] = adaptivity_laplace_BPX_fixed_refinement_all_decomp (problem_data, method_data, adaptivity_data, plot_data)
...
% SOLVE AND PLOT
  ...
  for ide = 1:numel(decomp)
    ...
    if (iter > 1)
    [u, bpx, CJA(iter), CGSA(iter), CA(iter), eig_A, eig_jac, eig_gs, niter_jac, niter_gs] = adaptivity_solve_laplace_BPX (hmsh, hspace, problem_data, method_data);
