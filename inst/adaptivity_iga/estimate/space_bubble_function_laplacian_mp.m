% SPACE_BUBBLE_FUNCTION_LAPLACIAN_MP: computation of the space struct for bubble function estimator on multipatch geometries
%
% USAGE:
%
%   space_bubble = space_bubble_function_laplace_mp (hmsh, degree)
%
% INPUT:
%
...
function sp = space_bubble_function_laplacian_mp (hmsh, degree, varargin)
...
% Build bubble functions as Bernstein polynomials on a reference element
  ...
  nqn_dir = hmsh.mesh_of_level(1).msh_patch{1}.nqn_dir;
...
%  sp_one_elem.shape_function_hessians = sp_one_elem.shape_function_hessians(:,:,:,bubbles(:),:);
  ...
  for iLevel = 1:hmsh.nlevels
    ...
    % Compute element lengths
      ...
      shifting_index = cumsum ([0 hmsh.mesh_of_level(iLevel).nel_per_patch]);
      other_index = 0;
      for iptc = 1:hmsh.npatch
        elems_on_patch = (shifting_index(iptc)+1):shifting_index(iptc+1);
        [~,~,active_on_patch] = intersect (hmsh.active{iLevel}, elems_on_patch);
        nel_on_patch = numel(active_on_patch);
        
        inds = cell (ndim, 1);
        [inds{:}] = ind2sub (hmsh.mesh_of_level(iLevel).msh_patch{iptc}.nel_dir, active_on_patch);

        length_univ = cellfun (@diff, hmsh.mesh_of_level(iLevel).msh_patch{iptc}.breaks, 'UniformOutput', false);

        indices = other_index + (1:nel_on_patch);
        for iDim = 1:ndim
          lengths(iDim,:,:,indices) = length_univ{iDim}(inds{iDim});
        end
        other_index = other_index + nel_on_patch;
      end
