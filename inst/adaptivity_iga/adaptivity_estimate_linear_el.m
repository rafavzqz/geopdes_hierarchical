...
for h = 1:hspace.ncomp
    ...
    for t=1:hspace.ncomp
        if t~=h
            partials_a = partials_a + squeeze(ders2(t,h,t,:,:)); %mixed derivatives of all components (except h-th component)
            partials_b = partials_b + squeeze(ders2(h,t,t,:,:)); %second derivatives of h-th component (except w.r.t. h-th variable)
        end
    end
    divergence(h,:,:) = (2*mu+lambda)*squeeze(ders2(h,h,h,:,:)) + (mu+lambda)*partials_a + mu*partials_b;
end
