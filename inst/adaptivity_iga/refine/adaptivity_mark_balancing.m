%ADAPTIVITY_MARK_BALANCING relax the mesh avoiding two level jumps at the element
%interface
%
%     [marked_elements] = adaptivity_mark_balancing (hmsh, marked_elements, ringIndex)
%
%
% INPUT:
%
...
function  [marked_elements]  = adaptivity_mark_balancing(hmsh, marked_elements, ringIndex)
...
for lev=hmsh.nlevels:-1:2
    ...
    for el=1:numel(active{lev})
        ...
        % if the neighbours are all either active or deactivated, i.e.
        % with children, then get the grandparents
        if (all(find(intersect(neighbours, active_deactivated{lev-1})))...
                && (lev>2))
            for neighbour=1:numel(neighbours)
                [grandparents, ~] = hmsh_get_parent(hmsh, lev-1, neighbours);
                %and add the active grandparents to the marked elements list
                if ~isempty(intersect(grandparents, active{lev-2}))
                    if (size(marked_elements{lev-2},2) == size(intersect(grandparents, active{lev-2}),2))
                        marked_elements{lev-2} = cat(1,marked_elements{lev-2},intersect(grandparents, active{lev-2}));
                    elseif (size(marked_elements{lev-2},1) == size(intersect(grandparents, active{lev-2}),1))
                        marked_elements{lev-2} = cat(2,marked_elements{lev-2},intersect(grandparents, active{lev-2}));
                    end
                end
            end
        end
