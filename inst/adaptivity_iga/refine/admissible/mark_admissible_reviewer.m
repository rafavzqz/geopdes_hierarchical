...
function [marked] = mark_admissible_reviewer (hmsh, hspace, marked, adaptivity_data)
...
% NASTY WAY TO CHECK OTHER VERTICES
    flag = 0;
    while (flag == 0)
      new_marked=[];
      all_funs = sp_get_basis_functions (hspace.space_of_level(lev), hmsh.mesh_of_level(lev), marked{lev});
    
      vertex_funs = intersect (all_funs, (hspace.space_of_level(lev).ndof-hspace.space_of_level(lev).ndof_vertices+1):hspace.space_of_level(lev).ndof);
      for ivert = 1:numel(hspace.space_of_level(1).vertices)
        funs_on_ivert = intersect (hspace.space_of_level(lev).dofs_on_vertex{ivert}, vertex_funs);
        cells_on_supp = sp_get_cells (hspace.space_of_level(lev), hmsh.mesh_of_level(lev), funs_on_ivert);
        new_marked = union (new_marked, cells_on_supp);
      end
      if (isempty (setdiff (new_marked, marked{lev})))
        flag = 1;
      end
