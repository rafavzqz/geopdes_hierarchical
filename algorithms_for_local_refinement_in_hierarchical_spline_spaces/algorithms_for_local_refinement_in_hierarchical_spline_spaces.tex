...
\begin{algorithmic}[1]
%\Statex \textbf{Input:} $\mesh, \basis, \{\marked_\ell\}_\ell$
\Statex \textbf{Input:} $\mesh, \basis, \{\marked_\ell\}$
\If{(Marking functions)}
%\State $\markedelements \gets \textbf{compute\_cells\_to\_refine} \, (\{\activeelements{\ell}\}_\ell, \{\marked_\ell\}_\ell)$
...
\EndIf
%\State $[\mesh, \{\newcells_\ell\}_\ell] \gets$ \textbf{refine\_hierarchical\_mesh}\, ($\{\markedelements_\ell\}_\ell$)
\State $[\mesh, \{\newcells_\ell\}] \gets$ \textbf{refine\_hierarchical\_mesh}\, ($\{\markedelements_\ell\}$)
%\State $\basis \gets \textbf{refine\_hierarchical\_space} \, (\mesh, \basis, \{\marked_\ell\}_\ell, \{\newcells_\ell\}_\ell)$
...
\begin{algorithmic}[1]
%\Statex \textbf{Input:} $\{\activeelements{\ell}\}_\ell, \{\functionstoremove_\ell\}_\ell$
...
\EndFor
%\Statex \textbf{Output:} $\{\markedelements_\ell\}_\ell$
...
\begin{algorithmic}[1]
%\Statex \textbf{Input:} $\mesh, \{\markedelements_\ell\}_\ell$
...
\begin{algorithm}
\caption{refine\_hierarchical\_space} \label{alg:refine_hierarchical_space}
\begin{algorithmic}[1]
%\Statex \textbf{Input:} $\mesh, \basis, \{\marked_\ell\}_\ell, \{\newcells_\ell\}_\ell$
\Statex \textbf{Input:} $\mesh, \basis, \{\marked_\ell\}, \{\newcells_\ell\}$
%\State $\{\functionstoremove_\ell\}_\ell \gets \textbf{compute\_functions\_to\_deactivate} \, (\mesh, \basis, \{\marked_\ell\}_\ell)$
\If {(Marking elements)}
  \State $\{\functionstoremove_\ell\} \gets \textbf{functions\_to\_deactivate\_from\_cells} \, (\mesh, \basis, \{\marked_\ell\})$
\ElsIf{(Marking functions)}
  \State $\{\functionstoremove_\ell\} \gets \textbf{functions\_to\_deactivate\_from\_neighbors} \, (\mesh, \basis, \{\marked_\ell\})$
\EndIf
\If {$\marked_{n-1} \not = \emptyset$} 
  \State $\basis \gets \textbf{add\_empty\_level} \, (n)$
\EndIf
\State $\texttt{REFINED}\_\basis \gets \textbf{update\_active\_functions} \, (\mesh, \basis, \{\functionstoremove_\ell\}, \{\newcells_\ell\})$ \Comment{Split in two (elements and functions)?}
...
\begin{algorithm}
\caption{functions\_to\_deactivate\_from\_cells} \label{alg:functions_to_deactivate_from_cells}
\begin{algorithmic}[1]
%\Statex \textbf{Input:} $\mesh, \basis, \{\marked_\ell\}_\ell$
\Statex \textbf{Input:} $\mesh, \basis, \{\marked_\ell\}$
\For {$\ell = 0, \ldots, n-1$}
  ...
  \ForAll {$\beta \in \functionstoremove_\ell}$}
    \If {(${\bf get\_cells}\, (\beta) \cap \activeelements{\ell} \not = \emptyset$)}
      \State $\functionstoremove_\ell \gets \functionstoremove_\ell \setminus \beta$
    \EndIf
  \EndFor
\EndFor
%\Statex \textbf{Output:} $\{\functionstoremove_\ell\}_\ell$
\Statex \textbf{Output:} $\{\functionstoremove_\ell\}$
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
\caption{functions\_to\_deactivate\_from\_neighbors} \label{alg:functions_to_deactivate_from_neighbors}
\begin{algorithmic}[1]
%\Statex \textbf{Input:} $\mesh, \basis, \{\marked_\ell\}_\ell$
\Statex \textbf{Input:} $\mesh, \basis, \{\marked_\ell\}$
\For {$\ell = 0, \ldots, n-1$}
  \State $\functionstoremove_\ell \gets \textbf{get\_neighbors} \, (\marked_\ell)$
  \State $\functionstoremove_\ell \gets (\functionstoremove_\ell \cap \activefunctions{\ell} ) \setminus \marked_\ell$ \Comment{Check only the support of non-marked functions.}
  \ForAll {$\beta \in \functionstoremove_\ell}$}
    \If {(${\bf get\_cells}\, (\beta) \cap \activeelements{\ell} \not = \emptyset$)}
      \State $\functionstoremove_\ell \gets \functionstoremove_\ell \setminus \beta$
    \EndIf
...
\begin{algorithmic}[1]
%\Statex \textbf{Input:} $\mesh, \basis, \{\functionstoremove_\ell\}_\ell$
\Statex \textbf{Input:} $\mesh, \basis, \{\functionstoremove_\ell\}$
\For {$\ell = 0, \ldots, n-1$}
  ...
  \If {(Marking elements)} 
    \ForAll {$\beta \in \tt{F^C}$}  \Comment{Mark new functions whose support is already refined}
      \If {(${\bf get\_cells}\, (\beta) \cap \activeelements{\ell} = \emptyset$)}
        \State $\functionstoremove_{\ell+1} \gets \functionstoremove_{\ell+1} \cup \beta$ 
      \EndIf
    \EndFor
  \EndIf
\EndFor
%\Statex \textbf{Output:} $\basis$ \Comment{The refined space, with updated $\{\activefunctions{\ell}\}}_\ell$ and $\{\deactfunctions{\ell}\}_\ell$
\Statex \textbf{Output:} $\basis$ \Comment{The refined space, with updated $\{\activefunctions{\ell}\}}$ and $\{\deactfunctions{\ell}\}$
\end{algorithmic}
\end{algorithm}
...
\For {$\ell = 0, \ldots, n-1$}
  ...
  \ForAll {$\beta \in \tt{F^C}$}
    \State $\tt{Q_\beta} \gets {\bf get\_cells} \, (\beta)$
%    \ForAll {$\tt{Q} \in \tt{Q}_\beta$}
      \If {$(\text {\rm any } (\tt{Q}_\beta \not \in \activeelements{\ell} \cup \deactelements{\ell}))$}
        \State $\tt{F^C} \gets \tt{F^C} \setminus \beta$
      \EndIf
%    \EndFor
  \EndFor
  \State $\activefunctions{\ell+1} \gets \activefunctions{\ell+1} \cup \tt{F^C}$
