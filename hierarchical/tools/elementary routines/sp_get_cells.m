...
function cell_indices = sp_get_cells (space, msh, fun_indices)
...
conn_indices = vertcat (conn_indices{:});
[~, cell_indices] = ind2sub ([space.nsh_max, msh.nel], conn_indices);
cell_indices = unique (cell_indices(:));
...
% 
% indices = cell (numel (fun_indices), 1);
...
%   indices{ifun} = sub2ind ([msh.nel_dir, 1], cells{:});
% end
% 
% indices = cellfun(@(x) x(:), indices, 'UniformOutput', false);
% cell_indices = unique (vertcat (indices{:}));
