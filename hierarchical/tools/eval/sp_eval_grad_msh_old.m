...
function [eu, F] = sp_eval_grad_msh_old (u, space, msh)
  ...
  weight = reshape (uc, [1, 1, 1, space.nsh_max, msh.nel]);
  
  space.shape_function_gradients = ...
         reshape (space.shape_function_gradients, space.ncomp, msh.rdim, ...
         msh.nqn, space.nsh_max, msh.nel);

  F(:,:,:) = msh.geo_map;
  eu(:,:,:,:) = reshape (sum (bsxfun (@times, weight, ...
            space.shape_function_gradients), 4), ...
            space.ncomp, msh.rdim, msh.nqn, msh.nel);
