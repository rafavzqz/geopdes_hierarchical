...
function [eu, F] = sp_eval_lapl_msh_old (u, space, msh)
  ...
  space.shape_function_hessians = reshape (space.shape_function_hessians, space.ncomp, ...
                              msh.rdim, msh.rdim, msh.nqn, space.nsh_max, msh.nel);
  shape_function_lapl = zeros (space.ncomp, msh.nqn, space.nsh_max, msh.nel);
  for ii = 1:msh.rdim
    shape_function_lapl = shape_function_lapl + ...
        reshape (space.shape_function_hessians(:,ii,ii,:,:,:), space.ncomp, msh.nqn, space.nsh_max, msh.nel);
  end
  
  F = msh.geo_map;
  eu = reshape (sum (bsxfun (@times, weight, ...
         shape_function_lapl), 3), space.ncomp, msh.nqn, msh.nel);
