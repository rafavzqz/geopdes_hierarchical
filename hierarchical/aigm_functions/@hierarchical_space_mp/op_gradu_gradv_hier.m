...
% 
%       K_lev = sparse (spv_lev.ndof, spu_lev.ndof);
%       for iptc = patch_list
...
%         end
%         dofsu = hspu.space_of_level(ilev).gnum{iptc};
%         dofsv = hspv.space_of_level(ilev).gnum{iptc};
%         K_lev(dofsv, dofsu) = K_lev(dofsv, dofsu) + ...
%             op_gradu_gradv (spu_lev.sp_patch{iptc}, spv_lev.sp_patch{iptc}, msh_ptc, coeff (x{:}));
%       end
%       dofs_u = 1:ndofs_u;
%       dofs_v = 1:ndofs_v;
%       
